...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p=='/api/stats':
   c=db.conn; total=int(c.execute('SELECT COUNT(*) FROM trades').fetchone()[0]); closed=int(c.execute('SELECT COUNT(*) FROM trades WHERE exit_time IS NOT NULL').fetchone()[0]); open_n=total-closed; wins=int(c.execute('SELECT COUNT(*) FROM trades WHERE pnl_sol>0').fetchone()[0]); pnl=float(c.execute('SELECT COALESCE(SUM(pnl_sol),0) FROM trades WHERE exit_time IS NOT NULL').fetchone()[0]); avg=float(c.execute('SELECT COALESCE(AVG(pnl_pct),0) FROM trades WHERE exit_time IS NOT NULL').fetchone()[0]); recent=[dict(r) for r in c.execute('SELECT symbol,entry_time,exit_time,pnl_pct,pnl_sol,exit_reason FROM trades ORDER BY entry_time DESC LIMIT 25').fetchall()]; self.j({'trades':closed,'wins':wins,'win_rate':round(wins/closed*100,2) if closed else 0,'pnl_sol':round(pnl,8),'avg_pnl_pct':round(avg,4),'open_trades':open_n,'recent':recent});return
